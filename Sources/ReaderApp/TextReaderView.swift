...
struct NativeTextView: UIViewRepresentable {
    ...
    func updateUIView(_ textView: UITextView, context: Context) {
        ...
        // Search: scroll to first match when query changes.
        if context.coordinator.lastSearchQuery != searchQuery {
            ...
            if !searchQuery.isEmpty {
                ...
                if range.location != NSNotFound {
                    context.coordinator.isScrollingProgrammatically = true
                    textView.scrollRangeToVisible(range)
                    context.coordinator.isScrollingProgrammatically = false
                    // Brief highlight of found text
    ...
    class Coordinator: NSObject, UITextViewDelegate, UIGestureRecognizerDelegate {
        ...
        // Uses character-based offset (offsetForCharProgress) for accuracy —
        // pixel-percentage was setting lastReportedProgress to the target before
        // layout was actually complete, blocking all subsequent syncProgress calls.
        private var pendingRestore: Double?
        func scheduleRestore(_ target: Double, in textView: UITextView) {
            pendingRestore = target
            lastReportedProgress = nil   // clear so syncProgress can correct if restore lands wrong
            attemptRestore(in: textView, retries: 25)
        }

        private func attemptRestore(in tv: UITextView, retries: Int) {
            guard let target = pendingRestore else { return }
            guard tv.bounds.width > 0, tv.textStorage.length > 0 else {
                if retries > 0 {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.08) { [weak self] in
                        self?.attemptRestore(in: tv, retries: retries - 1)
                    }
                }  else { pendingRestore = nil }
                return
            }
            // Character-based offset is exact. It forces layout up to the target
            // glyph, which is fast for plain-text and acceptable for images since
            // placeholder bounds were already set in Phase 1 of applyContent.
            let y = offsetForCharProgress(target, in: tv)
            let maxOffset = max(0, tv.contentSize.height - tv.bounds.height)
            let clamped = min(max(0, y), maxOffset)
            isScrollingProgrammatically = true
            ...
            // If the offset didn't stick (layout reset it), retry.
            if target > 0.001, abs(tv.contentOffset.y - clamped) > 20, retries > 0 {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.08) { [weak self] in
                    self?.attemptRestore(in: tv, retries: retries - 1)
                }
                return
